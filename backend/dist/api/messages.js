...
router.put("/:id", auth_1.requireAuth, async function (req, res) {
    try {
        ...
        if (!Number.isFinite(id))
            return res.status(400).json({ error: "Invalid id" });
        const { text } = (req.body ?? {});
        const existing = await db_1.default.message.findUnique({ where: { id } });
        if (!existing)
            return res.status(404).json({ error: "Not found" });
        if (existing.authorId !== userId)
            return res.status(403).json({ error: "Forbidden" });
        const message = await db_1.default.message.update({
            where: { id },
...
router.delete("/:id", auth_1.requireAuth, async function (req, res) {
    try {
        ...
        if (existing.authorId !== userId)
            return res.status(403).json({ error: "Forbidden" });
        await db_1.default.message.delete({ where: { id } });
